...
# For 21-day EWMA std
cleaned_data['ewma_std_return_21'] = cleaned_data.groupby('tic')['log_return'].ewm(
    span=21, adjust=False).std().reset_index(level=0, drop=True)

# For 252-day EWMA std
cleaned_data['ewma_std_return_252'] = cleaned_data.groupby('tic')['log_return'].ewm(
    span=252, adjust=False).std().reset_index(level=0, drop=True)

# Volatility regime as EWMA ratio
cleaned_data['volatility'] = cleaned_data['ewma_std_return_21'] / \
    cleaned_data['ewma_std_return_252']
...
cleaned_data['macd_std'] = cleaned_data.groupby('tic')['macd_normalized'].ewm(span=63, ignore_na=False,
                                                                              min_periods=1).std().reset_index(level=0, drop=True)

cleaned_data['macd'] = cleaned_data['macd_normalized']
...
optimizer = PortfolioOptimization(
    transaction_fee=0.003, comission_fee_model=None, vectorize=False, normalize='by_previous_time',
    tag="yyyyyyyyy", sharp_reward=False, last_weight=False, remove_close=False,
    add_cash=False, env=PortfolioOptimizationEnv
)
optimizer.train_model(train_data,
                      validation_data,
                      features=["close", "volatility", "macd", "rsi"
                                ],
                      model_name="ppo",
                      args={"n_steps":  256, "batch_size": 128, 'learning_rate': linear_schedule(2e-4),
                            'gamma': 0.95, 'gae_lambda': 0.85, 'ent_coef': 0.05},
                      policy_kwargs=dict(
